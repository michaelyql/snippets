...
- It prevents double-initialization. If oyu initialize variables inside the `{}` body, they are first default-initialized, then the value is assigned to them. The initializer list invokes the copy/move constructor directly. 
...
You can use a compile-time condition to make a constructor conditionally `noexcept`. This is useful for wrapper classes and templates.
```cpp
template <typename T>
class Wrapper {
    T value;
public:
    ...
    Wrapper(Wrapper&& other) noexcept(std::is_move_constructible_v<T> && std::is_nothrow_move_constructible_v<T>)
...
When you delete a derived object using a base class pointer, the compiler defaults to static binding. It only calls the destructor of the pointer's type (the base class).

A virtual destructor in C++ ensures that the destructor of a derived class is called when a derived object is deleted through a pointer to the base class. Without it, only the base class destructor executes, which causes resource leaks.

Adding the `virtual` keyword to the base class destructor forces dynamic binding. The runtime system looks up the actual object type and calls the derived destructor first, followed by the base destructor.
