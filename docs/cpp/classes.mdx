...
A `noexcept` constructor in C++ guarantees that the object initialization process will not throw any exceptions. noexcept, the runtime immediately calls `std::terminate`, aborting the program.
...
- Always pair a `noexcept` move constructor with a `noexcecpt` move assignment operator 
