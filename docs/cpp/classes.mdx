...
```
- C does not define `operator()` so it doesn't hide anything
    - The call to `operator()` here is ambiguous because both A and B define it 
    - Unqualified name lookup finds both `A::operator()` and `B::operator()` so the compiler doesn't know what to pick (this is **before** overload resolution takes place)
    - Fix by bringing both into `C`'s scope `using A::operator()` and `using B::operator()`
...
// e1 and c share address
// e2 cannot have same address as e1, so it uses another byte
```
...
// `B`'s foo does not override that of A since it declares const qualifier, 
// so it is a separate function on its own
// Calling `a->foo` still calls `A::foo` (nothing has been overridden)
```
...
```
- Note that for non-virtual functions, the derived class _hides_ the base class's function
    - When you call a non-virtual function from a derived object, the compiler searches and stops at the first function that it finds in the inheritance hierarchy with the matching function name, even if there is a better (or even exact) match above in the inheritance hierarchy. 
...
Other notes:
- Using `memset` directly on polymorphic objects is dangerous. See [C++ Object Model](./object-model#tldr)
...
```
- Only the destructor is allowed to be `virtual` since there is exactly one destructor 
- But templated member functions cannot be `virtual`, since each class has a vtable of fixed size but there are an unbounded number of instantiations of a template method, the vtable cannot know ahead of time how big it will be
...
- If the destructor is the pure virtual function,
    ...
    - The _most common use_ case of the destructor being pure virtual is when you want to create an _abstract base class_, but it doesn't have any natural functions that make sense to be pure virtual. In this case, the standard convention is to make the destructor pure virtual
...
- If you **declare** a copy constructor or copy assignment operator (or both), the move assignment and move operator are both not generated. The copy constructor/assignment operator is still generated if it is not declared. 
    - Declare means either declaring, defining, declaring `=default` or `=delete`
    - Vice versa, if you declare move ctor/assignment, the copy ctor/assignment is not generated unless you explicitly `=default` them or declare/define them
- The compiler only generates the move ctor/assignment if **none** of the five special member functions are _user-declared_ (copy ctor/assignment, move ctor/assignment, dtor)
    - If you declare _any_ of these five, the move operations are not auto-generated, unless you explicitly `= default` them
...
- Only declaring constructor doesn't implicitly disable generating move constructor
    - But declaring any of copy constructor, copy assignment, destructor tells the compiler you want to manually handle resources, so it avoids generating the move constructor (to avoid unsafe behaviour) 
- Best Practice
...
A member initializer list is used to initialize class fields _before_ the constructor body runs. It is separated from the constructor parameter list by a colon `:` and uses a comma-separated list. 
```cpp
class Player {
private:
    std::string name;
    const int id;       // Must use initializer list
    int& teamRef;       // Must use initializer list

public:
    // Member initializer list syntax
    Player(std::string p_name, int p_id, int& p_team) 
        : name(p_name), id(p_id), teamRef(p_team) {
        // Constructor body (runs after variables are initialized)
    }
};
```
- It is the **only** way to initialize `const` data members, reference variables (`&`), or base class constructors that lack a default constructor. 
- It prevents double-initialization. If you initialize variables inside the `{}` body, they are first default-initialized, then the value is assigned to them. The initializer list invokes the copy/move constructor directly. 
- Members are **always** initialized **in the order they are declared in the class definition**, not the order you list them in the constructor.

Additionally, a `std::initializer_list` constructor allows an object to be initialized using a _brace list of elements_. You must include `<initializer_list>` to use this feature.
```cpp
...
int main() {
    // Implicitly invokes the std::initializer_list constructor
    A a = {10, 20, 30, 40, 50};
}
```
- If a class provides a `std::initializer_list` constructor, the compiler will _aggressively prefer it_ over other constructors when curly braces are used. 
- Elements inside a `std::initializer_list` are immutable (`const`), and narrowing conversions (e.g. passing `double` when an `int` is expected) is a compile error.
- **All** elements in a `std::initializer_list` must be of the same type (or implicitly convertible to that single type)
    - If you use auto-deduction (like `auto list = {1, 2.5, 3};`), the compiler will typically fail to compile because it cannot deduce a single, consistent type `T` for all elements (e.g., mixing `int` and `double` in a single initializer list without an explicit common type will trigger a compilation error)
    - Elements can be of different types only if they can all be implicitly converted to the single underlying type `T` specified by the initializer list. For example, `std::initializer_list<double> list = {1, 2.5, 3};` is valid because the integer `1` and `3` are implicitly convertible to `double`.

### `noexcept` specifier

- A `noexcept` constructor in C++ guarantees that the object initialization process will not throw any exceptions. 
- It ensures memory safety, preventing resource leaks or partially constructed objects from entering undefined states.
- You can use a compile-time condition to make a constructor _conditionally_ `noexcept`. This is useful for wrapper classes and templates.
```cpp
template <typename T>
class Wrapper {
    T value;
public:
    // This constructor is noexcept only if T's move constructor is noexcept
    Wrapper(Wrapper&& other) 
        noexcept(std::is_move_constructible_v<T> && std::is_nothrow_move_constructible_v<T>)
        : value(std::move(other.value)) {}
};
```
- Do not use `noexcept` if a constructor allocates memory via `new` (unless using `std::nothrow`), as it can throw `std::bad_alloc`.
- Always pair a `noexcept` move constructor with a `noexcept` move assignment operator 
- Destructors are implicitly `noexcept` by default (since C++11)
...
```
- Technically UB to double destroy
- Manually calling destructor of an object with automatic (stack) storage almost always leads to UB unless you immediately "re-create" the object in the same storage using placement new
...
int main() {
    A* ptr = new A;
    ptr->fun(); // Ok, delete A object that's on the heap
}
```
- `delete` operator works only for objects allocated on the heap (using `operator new`)
- If the object is allocated on the stack, the behaviour is undefined

### Virtual destructor

- If destructor is not virtual, compiler defaults to static binding and only calls destructor of base class, but does not destroy derived subobject
- `virtual` destructor lets you delete a derived class object using a pointer to base class 
- `virtual` destructor forces dynamic binding, looks up actual object type and calls derived destructor first, followed by the base destructor

```cpp
class Base {
public:
    virtual ~Base() { /* Runs second */ }
};
...
public:
    ~Derived() override { delete[] data; } // Runs first!
};

Base* ptr = new Derived();
delete ptr; // Clean up is completely successful
```
- The rule of thumb is, if your class has at least one `virtual` function, it should almost certainly have a `virtual` destructor.
- Mark derived destructors with `override` to let the compiler check that it successfully overrides a base virtual destructor.
- You can use `virtual ~Base() = default;` if you do not need custom cleanup logic in the base class.
- A base class destructor should either be public and virtual, or protected and non-virtual
    - If you want to allow deletion through the base class interface, the base class destructor must be public and virtual
    - If you delete polymorphically without a virtual destructor, it is UB
    - Base classes don't always need to allow for polymorphic deletion. in such cases, make the destructor protected and non-virtual 

(See also [Abstract Base Class](#abstract-base-class-abc))
...
class Fixed_vector {
    ...
    public:
        ...
        // implicitly generates a copy ctor
...
int main() {
    ...
    // Compiles but prints nothing
}
```
- The compiler generates a copy constructor with the signature `Fixed_vector(const Fixed_vector&);` and that is a better match than the user-written templated constructor
    - A templated constructor is never considered a copy constructor under the C++ rules, and the compiler prefers the non-template (implicitly-declared) copy constructor when both are viable.
    - The compiler chooses the implicitly-generated non-templated copy constructor.
...
- By default, any constructor that can be called with a single argument acts as an implicit "converting constructor"
    - This includes ctors that _appear_ to have only one argument due to default arguments 
    - A temporary copy is made using the implicit converting constructor, then the copy/move ctor is called
...
- Tells compiler to generate the function without having to provide a definition.
    - For copy/move ctor/assignment operator, the default generated function does a member-wise copy/move
    - All data members (both primitive and user-defined) must be copyable/moveable in order for the compiler to generate the function 
    - If any data member is not copyable/mveable, the default copy/move ctor is **defined as deleted**
    - The **Rule of Zero** is encouraged: if your class simply owns objects that already manage their own resources correctly (e.g. `std::string`, `std::vector`, smart pointers), you can usually let the compiler generate all the special member functions using `= default`, or even omit them entirely
...
- They are the `&` and `&&` that appear after function parameters but before the function body
- `&` means that member function can only be called on lvalue objects
-  `&&` means the function can only be called on rvalue objects
- You can provide both versions to optimize behavior based on whether the object is an lvalue or rvalue (i.e. you can overload a function with just the ref qualifiers)
- `const` can be combined with `&` and `&&` to specify const lvalue or const rvalue (rarely used)
- const lvalues also binds to rvalues, so `const&` function can be called on rvalues
- When to use
	- There are niche cases when you'd need to optimize the memory footprint
	- A more practical case is when you're iterating over the result of a temporary object created in a loop
	- For example, `for (const auto& item : MyFactory().getList()) { ... }`
	- if` getList()` returns a reference to a member of the temporary returned by `MyFactory()`, you have undefined behavior.
	- because after `getList()` returns, the `MyFactory()` temporary is destroyed — but you're still iterating over a dangling reference or pointer
	- This would be safer if the factory had some method `makeList()` which returns it by value
- `static` methods cannot have `const` or ref qualifiers, because it does not have a `this` pointer 
- Destructors cannot have cv qualifiers (compile error)
...
int main() {
    U u{};
    u.ll = std::numeric_limits<long long>::max();
    std::cout << u.r.cooked;
}
```
- Reading from a union member different from the one most recently written is only permitted if the members share a **common initial sequence**, use one of the **explicit type punning cases** (reading unsigned char bytes), or you **reinterpret** the object with `std::bit_cast` (C++20)
- _Common initial sequence_ (CIS) is a specific exception to strict aliasing rules when it deals with active and inactive members inside a union
- If a standard-layout union contains multiple standard-layout structs, you are permitted to read a non-static data member of an inactive struct member, provided that the member is part of the CIS shared with the active struct member
...
int main() {
    std::cout << sizeof(storage); // 8
}
```
- The size of a union is the size of its largest member, plus any padding added by the compiler for alignment purposes. `char[5]` is 5 bytes, larger than the other data types when compared independently. Since we are aligning to 4 byte boundaries, the compiler inserts 3 bytes of padding.

### `std::variant`
```cpp
struct A {
    std::array<std::byte, 10> data_;
};

struct B {
    std::array<std::byte, 20> data_;
};

struct C {
    std::array<std::byte, 30> data_;
};

int main() {
    std::cout << sizeof(std::variant<A, B, C>); // > 30
}
```
- `variant` takes the size of the largest alternative data type 
    - It also takes at least one byte to keep track of the active alternative 
    - It may also insert padding bytes for alignment
